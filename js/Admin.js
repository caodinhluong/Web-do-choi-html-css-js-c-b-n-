i/* Admin.js - Dashboard Quản Trị ToyStore - CẬP NHẬT */
  (function () {
    'use strict';
    ...
    function renderCategoriesTable(list = state.categories) {
      ...
      if (list.length === 0) {
        ...
        renderPagination('categories', list.length);
        return;
      }
    ...
    function renderOrdersTable(list = state.orders) {
      ...
      if (list.length === 0) {
        ...
        `;
        renderPagination('orders', list.length);
        return;
      }
    ...
    function renderUsersTable(list = state.users) {
      ...
      if (list.length === 0) {
        ...
        renderPagination('users', list.length);
        return;
      }
    ...
    function renderPagination(tableType, totalItems) {
      ...
      if (pagination.currentPage > 1) {
        paginationHTML += `<button class="pagination-btn" data-page="${pagination.currentPage - 1}" data-table="${tableType}"><i class="fas fa-chevron-left"></i></button>`;
      }
      ...
      if (startPage > 1) {
        paginationHTML += `<button class="pagination-btn" data-page="1" data-table="${tableType}">1</button>`;
        if (startPage > 2) {
      ...
      for (let i = startPage; i <= endPage; i++) {
        ...
        paginationHTML += `<button class="pagination-btn${activeClass}" data-page="${i}" data-table="${tableType}">${i}</button>`;
      }
      ...
      if (endPage < totalPages) {
        ...
        paginationHTML += `<button class="pagination-btn" data-page="${totalPages}" data-table="${tableType}">${totalPages}</button>`;
      }

      // Next button
      if (pagination.currentPage < totalPages) {
        paginationHTML += `<button class="pagination-btn" data-page="${pagination.currentPage + 1}" data-table="${tableType}"><i class="fas fa-chevron-right"></i></button>`;
      }
    ...
    function searchUsers(term) {
      if (!term) {
        state.pagination.users.currentPage = 1;
        renderUsersTable();
        return;
      }
    ...
    function initEventDelegation() {
      ...
      document.addEventListener('click', function (e) {
        ...
        const tableType = paginationBtn.getAttribute('data-table');

        if (page && tableType && state.pagination[tableType]) {
